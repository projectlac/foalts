...
export class StoriesController {
  ...
  @ValidateQueryParam("storyId", { type: "number" }, { required: false })
  async readStories(ctx: Context) {
    const authorId = ctx.request.query.authorId as number | undefined;
    const storyId = ctx.request.query.storyId as number | undefined;

    let queryBuilder = Story.createQueryBuilder("story")
    ...
    if (storyId !== undefined) {
      queryBuilder = queryBuilder.where({ id: ctx.request.query.storyId });
    }
